...
const displayDetails = (req, res, criteria) =>{
	...
	client.connect((err) => {
        ...
        findRestaurant (db, DOCID, (docs)=>{
            ...
            res.status(200).render('rate',{doc:req.query,userName:req.session.username,resGrades:resGrades});
        })
    })
}
...
const gradeRestaurant = (req, res, criteria) =>{
    ...
    var docData = {};
    docData.grades = {};
    docData.grade = req.fields.grade;
    docData.grade.user = req.fields.user ;
    docData.grade.score = req.fields.score;
    console.log(docData);
    let DOCID = {};
    DOCID['_id'] = ObjectID(criteria._id);
    scoreRestaurant(DOCID, docData, (results)=>{
        console.log(results);
...
const deleteRestaurant = (doc, callback) =>{
    ...
    client.connect((err) => {
        ...
        console.log("Connected successfully to server");
        const db = client.db(dbName);
    db.collection('restaurants').deleteOne(doc, (err,results) => {
        assert.equal(err,null);
        console.log('deleteOne' + JSON.stringify(doc)+ 'was successful');
        callback();
        });
    })
}
...
const handle_Edit = (res, criteria) => {
    ...
    client.connect((err) => {
        ...
        let DOCID = {};
        DOCID['_id'] = ObjectID(criteria._id);
        findRestaurant(db, criteria, (docData) => {
...
const showMore =(req, res,criteria) =>{
    const client = new MongoClient(mongourl);
    client.connect((err) => {
        assert.equal(null, err);
        console.log("Connected successfully to server");
...
app.get('/delete',(req,res) =>{
	deleteRestaurant(req,res,req.query);
});
