...
app.post('/search',(req,res)=>{
	var doc = {};
	let currentTag = '/index';
	console.log(req.body.search);
	switch (req.body.type) {
		case 'name':
            currentTag = currentTag + '?name='+req.body.search;
		  break;
		case 'cuisine':
			currentTag = currentTag+ '?cuisine='+req.body.search;
			break;
		case 'borough':
			currentTag = currentTag + '?borough='+req.body.search;
		  break;
		default:
		  console.log("The tag is unavailable");
	  }
	 res.redirect(currentTag);
...
app.get('/api/restaurants/names/:name', (req,res) => {
    if (req.params.name) {
        ...
        res.status(500).json({"Error": "Missing criteria name"});
    }
})
...
app.get('/api/restaurants/borough/:borough', (req,res) => {
    if (req.params.borough) {
        ...
        res.status(500).json({"Error": "Missing criteria borough"});
    }
})
...
app.get('/api/restaurants/cuisine/:cuisine', (req,res) => {
    if (req.params.cuisine) {
        ...
        res.status(500).json({"Error": "Missing criteria cuisine"});
    }
})
